...
void PidTrdRunGetter::CalculateProbabilities(int nhits) {
  ...
  for (int truncmode = 0; truncmode < nhits + 1; truncmode++) {
    ...
    for (int ipdg = 0; ipdg < NumberOfPidsTrd - 1; ipdg++) {
      ...
      h2prob -> SetTitle(Form("dEdx : p_{rec} probability (P) (%s+) %d Trd Hits - %s", pid_codes_trd_.at(ipdg).second.Data(), nhits+1, histtitle_mode_.at(truncmode).Data()));
      particleprob.Update(ipdg, charge, nhits, truncmode, probmode, h2prob);
      getter_.AddParticleProb(particleprob);
      if (write_mchistograms_out_ == kTRUE)
	outFile_->cd(dirname_nhits_.at(nhits) + "/" + particlename + "/probP") && h2prob -> Write("",TObject::kOverwrite);
      ...
      h2prob -> SetTitle(Form("dEdx : p_{rec} probability (E) (%s+) %d Trd Hits - %s", pid_codes_trd_.at(ipdg).second.Data(), nhits+1, histtitle_mode_.at(truncmode).Data()));
      particleprob.Update(ipdg, charge, nhits, truncmode, probmode, h2prob);
      getter_.AddParticleProb(particleprob);
      if (write_mchistograms_out_ == kTRUE)
	outFile_->cd(dirname_nhits_.at(nhits) + "/" + particlename + "/probE") && h2prob -> Write("",TObject::kOverwrite);     
...
void PidTrdRunGetter::InitMcHistogramsOut() {
  ...
  array<TString, NumberOfProbMode> probname = {"probP", "probE"};
  ...
  for (int inhits = 0; inhits < NumberOfTrdLayers; inhits++) {
    ...
    for (int ipdg = 0; ipdg < NumberOfPidsTrd - 1; ipdg++) {
      ...
      directory2 = directory1->mkdir(probname.at(0));
      directory2 = directory1->mkdir(probname.at(1));
    }
  }
}
...
void PidTrdRunGetter::Init() {
  ...
  TString infilename = Form("%s/%s.%s", path_.Data(), jobId_.Data(), mcdata_name_.Data()); 

  inFile_ = new TFile(infilename,"READ");
  if (!inFile_ || !inFile_->IsOpen()) {
    throw runtime_error("Could not open input file: " + infilename);
  }
  
  OpenMcHistograms();
  
  if (write_mchistograms_out_ == kTRUE) {
    TString outfilename = Form("%s/%s_probabilities.root", path_.Data(), mcdata_name_.Data());
...
void PidTrdRunGetter::Exec() {
  ...
  outFile_getter->WriteObject(&getter_, getter_name_);
