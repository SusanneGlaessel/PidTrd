...
class PidTrdMcInput : public AnalysisTree::Task {
 ...
 protected:
  ...
  float GetPz(const AnalysisTree::BranchChannel& trd_track, float mom);
  ...
  std::string sim_tracks_name_{"SimParticles"};
  ...
  std::map<int, std::vector<TrdContainer>> trd_container_; //trd tracks sorted by number of hits
